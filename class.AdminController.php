...
class AdminController extends BaseController
{
    ...
    public function CreateUser(Request $request)
    {
        ...
        if (isset($data['username'], $data['password'], $data['role'])) {
            if ($this->dataAccess->AddUser($data['username'],
                $data['password'])
            ) {
                ...
                $this->dataAccess->AssignUserRole($data['username'],
                    $data['role']);
            } else {
                $html = $this->Render('Error.twig',
    ...
    public function LoginUser(Request $request)
    {
        ...
        if (isset($data['username'], $data['password'])) {
            if ($this->dataAccess->LoginUser($data['username'],
                $data['password'])
            ) {
    ...
    public function AssignRole(Request $request)
    {
        ...
        if (isset($data['username'], $data['role'])) {
            if ($this->dataAccess->AssignUserRole($data['username'],
                $data['role'])
            ) {
            ...
            $roles = $this->dataAccess->GetAllRoles();
            $users = $this->dataAccess->GetAllUsers();
            $html = $this->Render('AssignRole.twig',
                array('users' => $users, 'roles' => $roles));
        }
    ...
    public function CreateArticle(Request $request)
    {
        ...
        if (isset($data['header'], $data['text'])) {
            try {
                ...
                $this->dataAccess->AddArticle($_SESSION['username'],
                    $data['header'], $data['text'], $path);

                $articles = $this->dataAccess->GetAllArticles();

                $html = $this->Render('CreateArticle.twig',
                    ['articles' => $articles]);
